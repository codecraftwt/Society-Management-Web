...
export default function AdminAmenity() {
  ...
  const approveBooking = async (id) => {
    setApprovingId(id);
    try { await API.put(`/admin/amenities/bookings/${id}/approve`); await loadBookings(); }
    finally { setApprovingId(null); }
  };

  const rejectBooking = async (id) => {
    setRejectingId(id);
    try { await API.put(`/admin/amenities/bookings/${id}/reject`); await loadBookings(); }
  ...
  const filteredBookings = useMemo(() =>
    ...
    [bookings, searchBooking, bookingFilter]
  );
  ...
  const bStats = {
    ...
    paymentPending: bookings.filter(b=>b.status==="PAYMENT_PENDING").length,
        ...
        <>
          ...
          <div style={{ background:"var(--card-bg)", border:"1.5px solid var(--glass-border)", borderRadius:20, overflow:"hidden", boxShadow:"var(--shadow-sm)" }}>
            <div style={{ padding: isMobile ? "14px 16px" : "16px 22px", borderBottom:"1px solid var(--glass-border)", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              ...
              <span style={{ fontSize:12, color:"var(--text-secondary)" }}>{filteredBookings.length} of {bookings.length}</span>
            </div>
            ...
            {filteredBookings.length===0 ? (
              ...
              const isPast   = bDate && bDate < today;
              ...
              return (
                ...
                onMouseLeave={e => { e.currentTarget.style.background = isPaymentPending ? "rgba(139,92,246,0.03)" : "transparent"; }}>
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:8, flexShrink:0, width: isMobile && canAct ? "100%" : "auto" }}>
                    {canAct ? (
                      <>
                        <button onClick={() => approveBooking(b.id)} disabled={approvingId===b.id||rejectingId===b.id}
                          ...
                          {approvingId===b.id ? <Spinner cls="h-3 w-3"/> : <><MdCheckCircle size={14}/> {t("amenApprove")}</>}
                        </button>
                        <button onClick={() => rejectBooking(b.id)} disabled={approvingId===b.id||rejectingId===b.id}
                          ...
                          {rejectingId===b.id ? <Spinner cls="h-3 w-3"/> : <><MdCancel size={14}/> {t("amenReject")}</>}
                        </button>
                      </>
                    ) : (
