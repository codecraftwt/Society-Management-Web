...
export default function DeliveryEntry() {
  ...
  const loadFlats = async () => {
    try {
      ...
      console.error("Failed to load flats for delivery entry:", err);
      setFlats([]);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setForm({ agent_name: "", mobile: "", vehicle_number: "", flat_id: "" });
      setShowModal(false);
      loadDeliveries(1, debSearch, filter);
    } catch (err) {
  ...
  const emptyCopy = search || filter !== "ALL"
    ? (t("delEmptyFilter") || "No deliveries match your filters")
    : t("delEmpty");
    ...
    <div className="ge-root">
      ...
      <div className="ge-er">
        <div className="ge-er-left">
          ...
          <div>
            <h2 className="page-title">{t("delTitle")}</h2>
            <p className="page-subtitle">{counts.ALL} {t("delTotal") || "total deliveries"}</p>
          </div>
        </div>
      ...
      <div className="ge-toolbar">
        <div className="ge-search-wrap">
          ...
          <input
            ...
            placeholder={t("delSearch") || "Search agent, vehicle..."}
            value={search}
            ...
            <table className="ge-table">
              <thead>
                <tr className="ge-t-row">
                  ...
                  <th className="ge-th">{t("delColAgent")}</th>
                  <th className="ge-th">{t("delColPhone")}</th>
              ...
              <div key={v.id} className="ge-mobile-card">
                ...
                <div className="ge-mc-rows">
                  <div className="ge-mc-row">
                    <span className="ge-mc-label">{t("delColPhone")}</span>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={t("delModalTitle")}>
