  ...
  return (
    <div className="ad-overlay" onClick={() => !loading && onCancel()}>
      <div className="modal-box ad-modal animate-scaleIn" onClick={e => e.stopPropagation()}>
        ...
        <h3 className="ad-modal-title">Delete Document</h3>
        <p className="ad-modal-body">
          <span className="ad-modal-filename">"{doc.title}"</span> will be permanently deleted and cannot be recovered.
        </p>
        <div className="ad-modal-actions">
          ...
          <button className="ad-modal-confirm" onClick={onConfirm} disabled={loading}>
            {loading
              ? <><span className="ad-spinner" /> Deleting…</>
...
export default function CommitteeDocuments() {
  ...
  const catLabel = cat => ({
    All: "All", Legal: "Legal", Meetings: "Meetings",
    Guidelines: "Guidelines", Finance: "Finance", Security: "Security",
  }[cat] || cat);
  ...
  const handleUpload = async e => {
    ...
    try {
      ...
      setFile(null);
      setUploadOpen(false);
      showToast(`"${res.data.data.title}" uploaded successfully.`);
      fetchDocuments(1, debSearch, activeCat);
    } catch (err) {
    ...
    <div className="ad-root animate-fadeIn">
      ...
      <div className="ad-er">
        <div className="ad-er-left">
          ...
          <div>
            <h1 className="ad-page-title">Society Documents</h1>
            <p className="ad-page-subtitle">Upload and manage official society documents</p>
          </div>
        </div>
      ...
      <div className="ad-stats-row">
        <StatCard value={initialLoad ? "—" : counts.All}      label="Total"     variant="indigo" />
        <StatCard value={initialLoad ? "—" : counts.Legal}    label="Legal"     variant="purple" />
        <StatCard value={initialLoad ? "—" : counts.Finance}  label="Finance"   variant="green"  />
        <StatCard value={initialLoad ? "—" : counts.Meetings} label="Meetings"  variant="amber"  />
      </div>
        ...
        <div className="bg-card ad-upload-card">
          <div className="ad-card-er">
            <div className="ad-card-er-left">
              ...
              <div>
                <h2 className="ad-card-title">Upload New Document</h2>
                <p className="ad-card-subtitle">PDF, Word, Excel and PowerPoint supported</p>
              </div>
            </div>
          ...
          <form className="ad-form" onSubmit={handleUpload}>
            <div className="ad-form-grid">
              <div className="ad-field">
                <label className="ad-label">Title <span className="ad-req">*</span></label>
                <input className="input" type="text" placeholder="e.g. AGM Meeting Notes 2024"
            ...
            <div className="ad-field">
              <label className="ad-label">
                Description <span className="ad-opt">(optional)</span>
              </label>
            ...
            <div className="ad-field">
              <label className="ad-label">File <span className="ad-req">*</span></label>
              <div
                ...
                {file ? (
                  ...
                  <div className="ad-dz-empty">
                    ...
                    <p className="ad-dz-text">
                      Drag & drop or <span className="ad-dz-browse">browse files</span>
                    </p>
                    <p className="ad-dz-hint">PDF, DOC, XLS, PPT — max 10 MB</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="ad-form-footer">
              <button type="submit" className="btn-primary ad-submit-btn" disabled={uploading}>
                {uploading
                  ? <><span className="ad-spinner ad-spinner--white" /> Uploading…</>
                  : <><MdCloudUpload size={16} /> Upload Document</>}
              </button>
            </div>
          </form>
        </div>
      ...
      <div className="ad-list-wrap">
        <div className="ad-list-top">
          <h2 className="ad-list-title">All Documents</h2>
          {!initialLoad && (
            ...
            <table className="ad-table">
              <thead>
                <tr className="ad-t-row">
                  <th className="ad-th">Document</th>
                  <th className="ad-th ad-col-cat">Category</th>
                  <th className="ad-th ad-col-meta">Size</th>
                  <th className="ad-th ad-col-meta">Date</th>
                  <th className="ad-th ad-th-actions">Actions</th>
                </tr>
              </thead>
        ...
        {!initialLoad && !error && docs.length === 0 && (
          <div className="ad-empty">
            ...
            <p className="ad-empty-text">
              {counts.All === 0 ? "No documents uploaded yet" : "No documents found"}
            </p>
            <p className="ad-empty-sub">
              {counts.All === 0 ? "Upload the first document using the button above." : "Try a different search or filter."}
            </p>
                        ...
                        <td className="ad-td">
                          <div className="ad-row-actions">
                            <a href={fileUrl(doc)} target="_blank" rel="noopener noreferrer"
                              ...
                              <span className="ad-btn-label">View</span>
                            </a>
