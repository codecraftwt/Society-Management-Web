...
export default function SuperAdminFinancialReport() {
  ...
  useEffect(() => { fetchBills(1, "ALL", "", "", "", "", "", "", true); }, []);
  ...
  const applyFilter = () => {
    ...
    fetchBills(1, pendingSocietyId, pendingBlockId, pendingFloorId, pendingFlatId, pendingStatus, pendingFrom, pendingTo);
  };
  ...
  const handlePageChange = p => fetchBills(p, societyId, blockId, floorId, flatId, status, fromDate, toDate);

  const paidLabel = t("billPaid") || "Paid";
  const pendingLabel = t("billPending") || "Pending";
  const filterLabels = { 
  ...
  const handleExcelExport = async () => {
    ...
    exportToExcel({ fileName: "System_Financial_Report", sheetName: "Finances", data: all.map((b, i) => ({ "Sr No": i + 1, "Society": b.Flat?.Block?.Society?.name || "-", "Resident": b.Flat?.User?.name || "-", "Block / Flat": `${b.Flat?.Block?.name || "-"} / ${b.Flat?.flat_number || "-"}`, "Amount": b.amount, "Status": b.status, "Date": formatDate(b.created_at) })) });
  };
  const handlePDFExport = async () => {
    const all = await fetchAllForExport();
    exportToPDF({ title: "System Financial Report", fileName: "System_Financial_Report", columns: ["Sr No", "Society", "Resident", "Block / Flat", "Amount", "Status", "Date"], rows: all.map((b, i) => [i + 1, b.Flat?.Block?.Society?.name || "-", b.Flat?.User?.name || "-", `${b.Flat?.Block?.name || "-"} / ${b.Flat?.flat_number || "-"}`, b.amount, b.status, formatDate(b.created_at)]) });
  };
    ...
    <div className="page-root animate-fadeIn" style={{ overflowX: "hidden" }}>
      ...
      {!loading && counts.total > 0 && (
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(4,1fr)", gap: 10 }}>
          {[
            { label: t("rptCollected") || "Total Collected", val: `₹${counts.collected.toLocaleString()}`, color: "green" },
            { label: t("rptDue") || "Total Due", val: `₹${counts.due.toLocaleString()}`, color: "amber" },
            { label: t("billStatPaid") || "Paid Bills", val: counts.paid, color: "green" },
            { label: t("billStatPending") || "Pending Bills", val: counts.pending, color: "purple" },
          ].map((s, i) => (
      ...
      <div className="data-table-wrap" style={bleed}>
        ...
        {loading ? (
              ...
              <div key={b.id} className="animate-fadeIn" style={{ animationDelay: `${i * 25}ms`, background: "var(--chip-bg,rgba(255,255,255,0.04))", border: "1px solid var(--glass-border)", borderRadius: 12, overflow: "hidden" }}>
                <div style={{ padding: "11px 13px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                     <span style={{ fontSize: 15, fontWeight: 700, color: "var(--text-primary)" }}>₹{Number(b.amount).toLocaleString()}</span>
                     <PaymentBadge status={b.status} paidLabel={paidLabel} pendingLabel={pendingLabel} />
                  </div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    <span style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 11, color: "var(--text-secondary)" }}><MdBusiness size={12} style={{ color: "var(--accent,#6366f1)", flexShrink: 0 }} />{b.Flat?.Block?.Society?.name || "—"}</span>
                    <span style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 11, color: "var(--text-secondary)" }}><MdPerson size={12} style={{ color: "var(--accent,#6366f1)", flexShrink: 0 }} />{b.Flat?.User?.name || "—"}</span>
                    <span style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 11, color: "var(--text-secondary)", opacity: 0.7 }}><MdCalendarToday size={11} style={{ flexShrink: 0 }} />{formatDate(b.created_at)}</span>
                  </div>
                </div>
              </div>
            ))}
          ...
          <table className="data-table">
            <thead><tr>{[t("rptColSrNo") || "Sr No", t("profileTileSociety") || "Society", t("rptColResident") || "Resident", t("rptColFlat") || "Block / Flat", t("rptColAmount") || "Amount", t("billStatusCol") || "Status", t("vlColDate") || "Date"].map(h => <th key={h}>{h}</th>)}</tr></thead>
            <tbody>
              {bills.map((b, i) => (
                <tr key={b.id} className="animate-fadeIn" style={{ animationDelay: `${i * 15}ms` }}>
                  ...
                  <td><span style={{ fontSize: 13, fontWeight: 600, color: "var(--text-primary)" }}>{b.Flat?.Block?.Society?.name || "—"}</span></td>
                  <td><span style={{ fontSize: 13, color: "var(--text-secondary)" }}>{b.Flat?.User?.name || "—"}</span></td>
                  <td><span className="info-chip">{`${b.Flat?.Block?.name || "-"} / ${b.Flat?.flat_number || "-"}`}</span></td>
                  <td><span style={{ fontSize: 14, fontWeight: 700, color: "var(--text-primary)" }}>₹{Number(b.amount).toLocaleString()}</span></td>
                  <td><PaymentBadge status={b.status} paidLabel={paidLabel} pendingLabel={pendingLabel} /></td>
                  <td><span style={{ fontSize: 12, color: "var(--text-secondary)", whiteSpace: "nowrap" }}>{formatDate(b.created_at)}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
